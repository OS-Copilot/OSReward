...
invoke_docker_container() {
    ...
    # Mount local bert-base-uncased for GroundingDINO (offline HuggingFace)
    bert_model_path="$SCRIPT_DIR/../bert-base-uncased"
    bert_model_path=$(getrealpath $bert_model_path)
    if [ -d "$bert_model_path" ]; then
        docker_command+=" -v ${bert_model_path}:/client/bert-base-uncased:ro"
        echo "Using local BERT model: $bert_model_path"
    else
        echo "Warning: local BERT model not found at $bert_model_path (GroundingDINO may try HuggingFace download)"
    fi
